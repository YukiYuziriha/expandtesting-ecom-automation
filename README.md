...
### Одно предложение
> Фреймворк для UI‑автотестов на Python (Playwright + Pytest), который гоняет полный e‑commerce‑путь пользователя на [ExpandTesting.com](https://practice.expandtesting.com/) с параллельным запуском, кешированным логином и артефактами в CI/CD.

---

## ► Чем этот проект важен

Здесь не просто куча тестов под один сайт, а цельный пример того, как собрать удобный, поддерживаемый и CI‑дружелюбный E2E‑фреймворк под реальный e‑commerce сценарий. На его примере видно, что автор умеет:

* **Строить архитектуру**: строгий Page Object Model (POM) — тесты описывают сценарии и проверки, а Page‑объекты хранят локаторы и действия. Так код легче читать и расширять.
* **Жить с CI/CD**: весь набор тестов гоняется в параллельном GitHub Actions‑пайплайне на каждый PR, и при падении сразу есть видео и trace.
* **Делать надёжные локаторы**: селекторы строятся по устойчивым атрибутам (`role`, `data-testid`, `type=submit`), а не по тексту или CSS‑классам, поэтому мелкие правки верстки реже ломают тесты.
* **Ускорять прогоны**: сессионная фикстура авторизации (`storage_state` + `FileLock`) убирает повторный логин и сокращает время прогона примерно на 90% по сравнению с логином в каждом тесте.
...
| :-- | :-- |
| **Page Object Model (POM)** | Строгое разделение ответственности: тесты описывают сценарии и проверки, а Page‑объекты содержат локаторы и действия. Это делает тесты понятными и упрощает поддержку. |
| **Кешированная аутентификация** | Сессионная Pytest‑фикстура логинится **один раз за сессию**, сохраняет состояние в файл и создает новый контекст для каждого теста — так и быстро, и каждый тест изолирован. |
| **Надёжные локаторы** | Локаторы опираются на стабильные атрибуты DOM, которые описывают назначение элемента: семантический HTML (`form`, `button[type='submit']`), контракты доступности (`aria-label`) и `data-testid`, а не текст или CSS. |
| **CI/CD‑пайплайн** | PR запускает полный E2E‑набор в матрице браузеров (Chromium + Firefox), а push — ускоренный smoke на Chromium. Все джобы при падении прикладывают trace/video. Линтер (`ruff`) и типизация (`mypy`) ловят регрессии до мерджа. |
| **Логирование прогонов в SQLite** | Каждый прогон pytest пишет метаданные в `data/test_results.db` (SQLite); скрипт `scripts/ci/sqlite_observability.sh` подчищает файл, а CI выгружает БД и артефакты при ошибках. См. `docs/sqlite_observability.md`. |
| **Блокировка рекламы и трекеров** | Сетевой блок запросов плюс закрытие баннеров в DOM дают стабильную и «чистую» среду для UI‑тестов. |
| **Группы тестов по маркерам** | Все тесты помечены маркерами pytest (`smoke`, `ui`, `api`, `e2e`, `bookstore`, `notes`), поэтому в CI и локально можно быстро запускать только нужные группы сценариев. |
...
Зачем это нужно: быстрый smoke на push ускоряет итерации и меньше страдает от флейков внешнего сайта. На PR гоняется полный набор в несколько браузеров, поэтому перед мерджем есть нормальный уровень уверенности. `seq_only` собирает сценарии (например, logout), которые могут ломать сессии; отдельные пользователи и последовательный прогон для них сохраняют изоляцию и не растягивают общее время.
