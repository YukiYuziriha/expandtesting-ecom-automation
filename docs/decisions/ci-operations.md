## CI/CD: операционные решения (кратко)

- Контекст
  - GitHub Actions оркестрирует линтинг, типизацию и Playwright-наборы.
  - Цели: держать `main` зеленой, получать фидбек до 6 минут на push, собирать артефакты при сбоях, минимизировать флейки при сохранении сигнала.
  - Pre-commit локально запускает Ruff и MyPy; сканирование секретов включено.

- Область событий (текущее)
  - push (любая ветка): запускаются только smoke-срезы
    ...
    - Обоснование: быстрый фидбек и меньше флейков от внешних сайтов; исключаются `seq_only`, чтобы не ломать общее состояние.
  - pull_request → main: полный набор
...
- Матрица браузеров и последовательность
  - Bookstore UI: параллельная матрица (Chromium + Firefox) для всех тестов; спец-последовательность не нужна.
  - Notes UI: параллельность с изоляцией для детерминизма и скорости:
    ...
    - Обоснование: logout Notes может иногда инвалидировать сессии в параллели. Выделение `seq_only` с отдельными аккаунтами убирает перекрестные эффекты, сохраняя параллельность.
...
- Рассмотренные варианты
  - Одна последовательная джоба с линтом и тестами: проще, но медленно; слабая параллельность.
  - Всегда запускать полные матрицы на push: больше флейков и долгий runtime.
  - Ночные матрицы + push-smoke: быстрее push, но риск регрессий на PR.

- Решение
  - Придерживаться стратегии push-smoke и PR-full, с Notes-спецификой по последовательным легам.
...
- Последствия
  ...
  - ✅ Полный confidence перед merge через PR-матрицы и параллельный последовательный лег.
  - ✅ Любые сбои сопровождаются артефактами для диагностики.
  - ⚠️ Чуть сложнее YAML и больше параллельных раннеров из-за дополнительного последовательного лего.
